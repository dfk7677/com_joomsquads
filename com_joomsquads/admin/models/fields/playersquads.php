...
class JFormFieldplayersquads extends JFormField {
	...
	public function getInput() {
		...
		$p_id = $this->form->getValue ( 'id' );
		if (null !== $p_id) {
			$html = '<table><tr><th>Squad</th><th>Is member?</th><th>Position</th>' . '<th>Ordering</th</tr>';
			$i = 0;
			...
			foreach ( $results as $value ) {
				...
				try {
					...
					$query->where ( $db->quoteName ( 'player_id' ) . ' = ' . $p_id . ' AND ' . $db->quoteName ( 'squad_id' ) . ' = ' . $value->id );
					$db->setQuery ( $query );
					$row = $db->loadAssoc ();
					$db->transactionCommit ();
				} catch ( Exception $e ) {
    ...
    <td> <input type="checkbox" name="squad[]" value="' . $value->id . '" ' . $checked . ' id="ch' . $i . '" 
  ...
  <td> <input type="number" min="1" max="100" name="ordering[]" 
        style="width: 30px;" value="' . $row ['squad_ordering'] . '" ' . $disabled . ' id="ord' . $i . '"></td>
  </tr>';
				$i ++;
			}
			$html .= '</table>';
		}
		else {$html='Please add squads by editing the player';}
		return $html;
	}
}
